...
/*
**  @class SushiSalesYesterday
*/
class SushiSalesII extends PageLayoutA
{
    ...
    {
        $dayOfWeek = FormLib::get('dayofweek', 'yesterday');
        $ret = '';
        $dbc = scanLib::getConObj();
        $stores = array(1, 2);
        $dates = array();

        $lastYesterday = date('Y-m-d', strtotime($dayOfWeek));
        $dates[] = $lastYesterday;
        $date = new DateTime($dayOfWeek);
        $days = array();
        $days[] = $date->format('l');
